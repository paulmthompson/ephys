...
def apply_zca_whitening(voltage_matrix, epsilon=10.0, rescale_amplitude=True, robust_cov=True):
    ...
    centered_volt = voltage_matrix - median_vars
    ...
    robust_std = np.median(np.abs(centered_volt), axis=1, keepdims=True) * 1.4826
    ...
    if robust_cov:
        ...
        is_clean_sample = np.all(np.abs(centered_volt) < (4 * robust_std), axis=0)
        clean_centered_volt = centered_volt[:, is_clean_sample]
        cov = np.cov(clean_centered_volt)
    else:
        cov = np.cov(centered_volt)
    ...
    # 6. Apply ZCA
    voltage_zca = ZCA_matrix @ centered_volt
    ...
    if rescale_amplitude:
        ...
        voltage_zca = voltage_zca * fixed_scalar
